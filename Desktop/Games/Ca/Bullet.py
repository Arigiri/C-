...
class bullet(pygame.sprite.Sprite):
	...
	def update(self, bg, mc, game):
		...
		if self.type == "MAIN":
			...
			self.pos = (x, y)
			self.rect.center = (self.pos[0] + self.w/2, self.pos[1] + self.h/2)
			if (self.type == "mobs_0" or self.type == "mobs_3") and self.angle == 0:
	...
	def write(self, num, type):
		f = open("saves\\bullet" + self.type + str(num) + ".txt", "w")
		f.write(str(self))
		f.close()
	def read(self, num, type):
		f = open("saves\\bullet" + self.type + str(num) + ".txt", "r")
		read = f.readlines()
